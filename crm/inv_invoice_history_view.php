					...
					<td valign="top">
						...
						<table border="0" cellpadding="0" cellspacing="0" height="40" valign="bottom" width="1200">							
							...
							<tr class="ui-widget-header">
								...
								<td width="200" height="30"  style="padding-left:5px">	Builder Name </td>
						...
						<?php
							...
							$Query  = "SELECT * ";
							$Query .= " FROM invoice_manage WHERE 1=1 " . $s_Cond . "ORDER BY inv_no ASC LIMIT $start, $limitList";
						...
						<tr align="center" <?php echo $even_odd;?> onmouseover="javascript:this.style.color='#536499';this.style.backgroundColor='#dcebfe';"  onmouseout="javascript:this.style.color='#000000';this.style.backgroundColor='';">
							...
							<td class="left"><input type="hidden" value="<?=$list_Records[$i]["inv_no"]?>" name="inv_no[]"><b><?php echo getName("builder",$list_Records[$i]["builder_id"]); ?></td>
				
							<td class="left"><?php echo getName("project",$list_Records[$i]["project_id"]); ?></td>
							<td class="left"><?=getAUDate($list_Records[$i]["payment_due"])?></td>
							<td <?=$bgcolor?>><?=($list_Records[$i]["inv_amount"])?>&nbsp;</td>
							<td class="left"><?=getAUDate($list_Records[$i]["received_date"])?></td>
							
							<td class="right"><?=($list_Records[$i]["received_amount"])?>&nbsp;</td>
							<td class="right"><?=($list_Records[$i]["retention_amount"])?>&nbsp;</td>
							<td class="right"><?=($list_Records[$i]["diff_amount"])?>&nbsp;</td>
