...
if($act=="delete") {
	...
	$sql = "SELECT COUNT(*) FROM employee WHERE visa_id='" .$visa_id."'";	
	$row = getRowCount($sql);	
	if($row[0] > 0) {
		echo "<script>alert('It can not delete this visa name because it already has been used in employee infomation');history.back();</script>";
	} else {
		$sql = "DELETE FROM visa WHERE visa_id='".$visa_id."' ";
		pQuery($sql,"delete");
	
		$sql ="DELETE FROM history WHERE history_table = 'visa' AND history_table_id = '".$visa_id."' ";
		pQuery($sql,"delete");
			...
			<tr>
				...
				<tr>
					...
					<table border="0" cellpadding="0" cellspacing="0" width="1000" class="font11_bold">
						...
						<tr><td width="252"><img src="images/icon_circle03.gif">&nbsp;<span style="height:21px">Visa List</span></td></tr>
				...
				<tr>
					<td valign="top">
						...
						<?
							...
							$limitPage = 5;
							$limitList = 20;
							$total = getRowCount2("SELECT COUNT(*) FROM visa WHERE 1=1 ". $s_Cond);
							//echo ceil(1.2);
							...
							$Query  = "SELECT * ";
							$Query .= " FROM visa ORDER BY visa_name ASC LIMIT $start, $limitList";
						...
						<tr  class='ui-widget-content' align="center">
							<td><?=$total - (($limitList * ($page-1)) + $i)?><input type="hidden" value="<?=$list_Records[$i]["visa_id"]?>" name="visa_id[]"></td>
							<td><a href="visa_regist.php?visa_id=<?=$list_Records[$i]["visa_id"]?>&action_type=modify"><b><?=$list_Records[$i]["visa_name"]?></b></a></td>
							<td><a href="javascript:if(confirm('Are you sure?')) { location.href='<?=$_SERVER['PHP_SELF']?>?visa_id=<?=$list_Records[$i]["visa_id"]?>&act=delete';}"><img src="zb/images/x.gif"></a></td>
						</tr>
						<?
							}
							} else {
						...
						<tr>
							<td></td>
							<td align="right"><input type="button" value="New Entry" onclick="location.href='visa_regist.php'"></td>
						</tr>
						</table>
						<br>
