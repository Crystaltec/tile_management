...
if($action_type=="modify") {
	...
	// wage 삭제 sql 문 추가 
	pQuery($sql, "delete");
	echo "<script>alert('EMPLOYEE IS ALREADY DELETED!');location.href='employee_list.php';</script>";
	exit;
}
...
$(function() {
	...
	$(".current_ft_wage").click(function() {
		...
		alert("Attention!\nMake sure to add first the Job Management before \nEditing the Full time wage\nIf you did not add first the Job order list\nIt will overwrite the previous wage");
					...
					<td valign="top">
						...
						<tbody>
							...
							<input type="text" size="15" name="phone_number" value="<?=$list_Records[0]["phone_number"]?>">
                                                        ...
                                                        <td id='current_ft'>
							Current: <input type='text' name='current_ft_wage' id='current_ft_wage'  <?php if ($action_type <> '') echo " readonly='readonly' ";?>  class='current_ft_wage' size='6' value='<?php echo getValue('wages','wages_amount'," and wages_id = '".$list_Records[0]['current_ft_wage_id']."' ")?>'>
					
                                                        <input type='hidden' name='current_ft_wage_id' id='current_ft_wage_id' value='<?php echo $list_Records[0]['current_ft_wage_id'];?>' ></td>	
							

                                                        <td id='current_hr'>
							Current: <input type='text' name='current_hr_wage'  id='current_hr_wage'   <?php if ($action_type <> '') echo " readonly='readonly' ";?>  size='6' class='current_hr_wage' value='<?php echo getValue('wages','wages_amount'," and wages_id = '".$list_Records[0]['current_hr_wage_id']."' ")?>'>
						...
						<br>
						<div class="right">
						<input type="button" value="Save" onclick="javascript:if(confirm('Are you sure you want to save this?')){formchk();}"><input type="button" value="Delete" onclick="javascript:if(confirm('Are you sure you want to delete this?')) { location.href='<?=$_SERVER['PHP_SELF']?>?id=<?=$list_Records[0]["id"]?>&action_type=delete';}">
						</div>
					</td>
				</tr>
				</form>
