...
if ($_POST['action_type'] == 'add' ) {
	if ($_POST['new_value']) {
		$sql = "INSERT INTO visa_status (visa_status_name,account_id,regdate) VALUES ('".$_POST['new_value']."','$Sync_id','$now_datetimeano')";
		pQuery($sql,"insert");
	}
} else if ($_POST['action_type'] == 'del'){
	if ($_POST['selected_id']){
		$sql = "DELETE FROM visa_status WHERE visa_status_id = '".$_POST['selected_id']."'";
		pQuery($sql,"delete");
	}
}

$sql  = "SELECT visa_status_name, visa_status_id FROM visa_status ORDER BY visa_status_name";
$result = mysql_query($sql) or exit(mysql_error());
$new_opt = "<option value=''>Please Select</option>";
$msg='';
if ($result) {
	while ($row = mysql_fetch_assoc($result)) {
		$msg .= "<span>" . $row['visa_status_name']. "<span onclick='form_delete(".$row[visa_status_id].")' class='ui-icon ui-icon-close' style='display:inline-block !important;'></span></span><br />";
		$new_opt .="<option value=".$row['visa_status_id'].">".$row['visa_status_name']."</option>";
	}
} else {
...
$(document).ready(function() {
	...
	$(window.opener.document).find('#visa_status_id').html("<?php echo $new_opt;?>");
