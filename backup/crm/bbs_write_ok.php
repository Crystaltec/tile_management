...
// query insert ----------------------------------------------------------------------------------
// SELECT look number 
$sql = "SELECT MAX(look) as look FROM ".$_BOARDID;
...
if ($_BOARDID == "project_web" )
{
	$data = array("look"=>$look, "thread"=>$thread, "subject"=>$subject, "name"=>$name, "userid"=>$userid, "pass"=>$pass, "link"=>$link, "hide"=>$hide,"builder_id"=>$builder_id,"contents"=>$contents, "regdate"=>$now_datetimeano);
}
else if ($_BOARDID == "tiles" )
{
	$data = array("look"=>$look, "thread"=>$thread, "subject"=>$subject, "name"=>$name, "userid"=>$userid, "pass"=>$pass, "link"=>$link, "hide"=>$hide,"category_id"=>$tiles_category_id,"contents"=>$contents, "regdate"=>$now_datetimeano);
}
else if ( $_BOARDID == "nobel_stones" )
{
	$data = array("look"=>$look, "thread"=>$thread, "subject"=>$subject, "name"=>$name, "userid"=>$userid, "pass"=>$pass, "link"=>$link, "hide"=>$hide,"category_id"=>$nobel_stones_category_id,"contents"=>$contents, "regdate"=>$now_datetimeano);
}
else 
{
	$data = array("look"=>$look, "thread"=>$thread, "subject"=>$subject, "name"=>$name, "userid"=>$userid, "pass"=>$pass, "link"=>$link, "contents"=>$contents, "regdate"=>$now_datetimeano);
}
...
for($f=0;$f<$info_file_count;$f++){
	...
	if ($_FILES["file_up_".$f]["tmp_name"]) {
		...
		//$extra[$f] = $_FILES["file_up_".$f]["name"];
		$extra[$f] = $_BOARDID."_".$new_no."_".$f.strtolower(strrchr($_FILES["file_up_".$f]["name"],"."));
