...
function getSelectInfo(selectObj, index){	
	if(index == "1")
		selectbox = document.getElementById("dispSelect01");
	else
		selectbox = document.getElementById("dispSelect02");

	if(selectObj.value == "FF") {
		selectbox.innerHTML = "<input type='text' name='srch_custword' size='8'>";
		return;
	}
...
function createOptions(sel,obj) {
	...
	if(root.hasChildNodes()) {
		...
		for(var i=0; i < nds.length; i++) {
			opt = document.createElement("option");
			opt.setAttribute("value",nds[i].childNodes.item(0).firstChild.nodeValue+"/"+nds[i].childNodes.item(1).firstChild.nodeValue);
