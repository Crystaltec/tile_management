...
f.action="<?=$_SERVER['PHP_SELF']?>
	...
	$(function() {
		...
		$(".add_job").click(function() {
			...
			new_window = window.open('add_job.php?id=' + re_id + '&re_date=' + re_date + '&re_list_view=' + re_list_view, '', 'width=500,height=500,top=' + top1 + ',left=' + left1);
			if(window.focus) {
				new_window.focus();
			}
			return false;
		...
		$(".update_job").click(function() {
			...
			left1 = (screen.width / 2) - (500 / 2);
			top1 = (screen.height / 2) - (500 / 2);
			new_window = window.open('update_job.php?id=' + re_id, '', 'width=500,height=500,top=' + top1 + ',left=' + left1);
			if(window.focus) {
				new_window.focus();
			}
			return false;
								...
								<tr>
									...
									<form name="searchform">
										<table border="0" cellpadding="0" cellspacing="0" height="40" valign="bottom" width="1160">
											<tr class='ui-widget-header'>
												...
												<select id="visa_id" class="multiselect" multiple="multiple" name="visa_id[]" style="width:190px;">
													...
													while($id_rst = mysql_fetch_assoc($id_cnn)) {
														$str_select = "";
														if (in_array($id_rst['visa_id'],$visa_id)){
															$str_select= "selected";
														}
													?>
											...
											-->
											<tr>
												<td colspan="4" align="right" height="30"><span style="float:left;">Items per page:
										...
										<table class="list_table" cellspacing="1" cellpadding="0" border="0" style="width: 1160px;">
											<?php
											if ($list_view == 'p') {
											?>
											<thead>
												<th width="40">No</th>
												<th width="110"><span onclick="reSort('name')">Name</span></th>
												<?php
												/* 요일 가져오기 */
												...
												for ($i = 0; $i <= 6; $i++) {
													...
													$p_count = 0;
													if ($date_p == $date_count[$i]['job_date']) {
														$p_count = $date_count[$i]['count'];
													}
											...
											echo  "<td height='22'>".($total - (($limitList * ($page-1)) + $i))."</td>".
											"<td class='left' >".$list_Records[$i]["project_name"]."</td>";
											...
											echo "<span class='".strtolower($row['job_session'])." update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".$row['employee_name']. " ". $row['job_session'];
											...
											echo "<span class='job_extra_hour update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".$row['employee_name'] . " ". $row['job_extra_hour'] . "H";
											if($row['travel_fee'] <> 0 || $row["parking_fee"] <> 0) {
											echo " <span class='car'>C</span> ";
											}
											echo "</span>";
											...
											echo  "<td height='22'>".($total - (($limitList * ($page-1)) + $i))."</td>".
											"<td class='left' ><b>".$list_Records[$i]["employee_name"]."</b><br/>".$list_Records[$i]["contacts"]."</td>";
											...
											if ($row['attendance'] == 'A') {
											if ($row['job_session'] && $row['job_session_rates'] > 0) {
											echo "<span class='".strtolower($row['job_session'])." update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".
											...
											if ($row['job_extra_hour'] <> 0 && $row['job_extra_hour_rates'] > 0) {
											echo "<span class='job_extra_hour update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".
											...
											echo "<span class='".strtolower($row['job_session'])." update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".$row['project_name'] . " ". $row['job_session'] ;
											...
											if ($row['job_extra_hour'] <> 0 && $row['job_extra_hour_rates'] > 0) {
											echo "<span class='job_extra_hour update_job' id='".$row['id']."' >"."<span class='contact $call_class' id='call_".$row['id']."'>".$row['check_call']."</span>"."<span class='attendance $absence_class' id='att_".$row['id']."'>". $row['attendance']."</span> ".$row['project_name'] . " ". $row['job_extra_hour'] . "H";
